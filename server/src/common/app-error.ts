...
export class AppError extends Error {
    ...
    static badRequest(message: string, details?: Record<any, any>): AppError {
		return new AppError({ name: 'BadRequestError', message, statusCode: HttpCode.BAD_REQUEST, details });
	}

	static unauthorized(message: string, details?: Record<any, any>): AppError {
		return new AppError({ name: 'UnauthorizedError', message, statusCode: HttpCode.UNAUTHORIZED, details });
	}

	static forbidden(message: string, details?: Record<any, any>): AppError {
		return new AppError({ name: 'ForbiddenError', message, statusCode: HttpCode.FORBIDDEN, details });
	}

	static notFound(message: string, details?: Record<any, any>): AppError {
		return new AppError({ name: 'NotFoundError', message, statusCode: HttpCode.NOT_FOUND, details });
	}

	static internalServer(message: string, details?: Record<any, any>): AppError {
		return new AppError({ name: 'InternalServerError', message, statusCode: HttpCode.INTERNAL_SERVER_ERROR, details });
	}
}